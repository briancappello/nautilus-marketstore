"""
Example: Backtest using historical data from MarketStore.

Loads OHLCV bars from MarketStore and runs a simple bar-counting
backtest in NautilusTrader.
...
print(f"Loaded {len(bars)} bars from MarketStore")
...
class BarCounter(Strategy):
    ...
    def on_bar(self, bar) -> None:
        ...
        if self._count % 100 == 0:
            self.log.info(f"Processed {self._count} bars, latest close: {bar.close}")
